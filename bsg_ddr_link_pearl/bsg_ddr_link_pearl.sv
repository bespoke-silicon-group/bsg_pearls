...
module bsg_ddr_link_pearl
 ...
 #(parameter `BSG_INV_PARAM(tag_els_p)
   ...
   , parameter `BSG_INV_PARAM(io_data_width_p)
   , parameter `BSG_INV_PARAM(clk_gen_ds_width_p)
   , parameter `BSG_INV_PARAM(clk_gen_num_taps_p)
   , parameter `BSG_INV_PARAM(ddr_lg_fifo_depth_p)
   , parameter `BSG_INV_PARAM(ddr_lg_credit_to_token_decimation_p)
   , parameter `BSG_INV_PARAM(ddr_use_extra_data_bit_p)
  ...
  wire [tag_lg_els_lp-1:0] ddr_tag_node_offset_li = tag_node_id_offset_i + '0;
  wire [tag_lg_els_lp-1:0] clk_gen_tag_node_offset_li = ddr_gen_tag_node_offset_li + bsg_link_ddr_tag_local_els_gp;
  ...
  `declare_bsg_tag_client(ddr_uplink_reset_li, tag_lines_lo.uplink_reset, 1, io_clk_lo);
  `declare_bsg_tag_client(ddr_downlink_reset_unsync_li, tag_lines_lo.io_downlink_reset, 1, io_clk_lo);
  `declare_bsg_tag_client_unsync(ddr_async_token_reset_li, tag_lines_lo.io_async_token_reset, 1);
  `declare_bsg_tag_client(core_downlink_reset_li, tag_lines_lo.core_downlink_reset, core_clk_i);
  `declare_bsg_tag_client(core_uplink_reset_li, tag_lines_lo.core_uplink_reset, 1, core_clk_i);
